...
- USA:    
      ...
      - Filtros: Ocupados privados sin servicio doméstico (filtrado por rama)
...
Emp.rama.USA <- calculate_tabulates(Base_USA.c,"INDLY",weights = "ASECWTH") %>% 
  left_join(ramas_usa %>% mutate(INDLY = factor(IND)))
...
variables_comunes <- 
  c("Pais","IOP","PONDERA","PONDERA_SALARIOS",
...
```
#Tamaños empresas 
Más allá de la diferencia en la clasificación de tamaños de establecimientos, acá se puede ver la diferencia en la cantidad de personas que se ocupan en empresas de distintos tamaños entre ambos países. Argentina,  el 57% del empleo es en pequeñas, en USA,  más del 66% en grandes 
```{r}
base.unica %>% 
...
distrib.grupos<- base.unica %>% 
  ...
  summarise(ocupados=n(),
            ...
            tasa.asalarizacion = formato_porc(asalariados.pond/ocupados.pond*100),
  ...
  group_by(Pais) %>% 
  mutate(Particip_emp = formato_porc(ocupados.pond/sum(ocupados.pond)*100),
         Particip_emp_asalariad = formato_porc(asalariados.pond/sum(asalariados.pond)*100),
         tasa.subocup.horaria = formato_porc(subocup.horaria/ocupados.pond*100),
         tasa.subocup.horaria.asal = formato_porc(subocup.horaria.asal/asalariados.pond*100)) 
...
tabla.caracterizacion.1 <- distrib.grupos %>% 
  select(grupos.tamanio,grupos.nivel.ed,Pais,
         ocupados.pond,Particip_emp,tasa.asalarizacion) %>% 
  pivot_wider(names_from = Pais,values_from = c(ocupados.pond,Particip_emp,tasa.asalarizacion)) %>%  
  arrange(grupos.tamanio)
  
tabla.caracterizacion.2 <- distrib.grupos %>% 
  select(grupos.tamanio,grupos.nivel.ed,Pais,
         asalariados.pond,Particip_emp_asalariad,tasa.subocup.horaria.asal) %>% 
  pivot_wider(names_from = Pais,values_from = c(asalariados.pond,Particip_emp_asalariad,tasa.subocup.horaria.asal)) %>%  
  arrange(grupos.tamanio)  

names(tabla.caracterizacion.1)[3:ncol(tabla.caracterizacion.1)] <- c("ARG","USA")
names(tabla.caracterizacion.2)[3:ncol(tabla.caracterizacion.2)] <- c("ARG","USA")
```
##Tomando a todos los ocupados
```{r}
kable(tabla.caracterizacion.1, align = "r") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = "center") %>%
  add_header_above(c(" " = 2,
                     "Total Ocupados" = 2,
                     "Participación (% ocupados)" = 2,
...
##Sólo a asalariados
Todos los valores que aparecen acá son sólo para asalariados
```{r}
kable(tabla.caracterizacion.2, align = "r") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = "center") %>%
  add_header_above(c(" " = 2,
                     "Total Asalariados" = 2,
                     "Participación" = 2,
                     "Tasa de subocup. horaria" = 2))
```

##Escalera Salarial
###Niveles educativos y tamaños
...
ingreso.promedio <- base.unica %>% 
  ...
  summarise(casos = n(),
            ...
            coef.variacion = w.cv(IOP,PONDERA_SALARIOS),
            salario.mensual.mediana = median(IOP),
            suboc.horaria = sum(PONDERA[precario.part=="Precario"],na.rm = T)/
                          sum(PONDERA,na.rm = T)) %>% 
...
En cuanto a calificaciones por ahora solo pude ver lo de argentina, dado que el clasificador del a CPS no permite delimitar ocupaciones según calificación fácilmente.
Estuve investigando y habría que hacer dos *crosswalks*, uno para transformar las ocupaciones a los códigos del SOC (Clasif. utilizado en censos de USA) y otro para transformarlas a ISCO (Europa)     

Lo que se puede ver para los datos de argentina es que el corte por calificación p/ asalariados da una escalerita muy similar a la de los niveles educativos propuestos.
```{r}
salario.promedio.calif <- eph.ocup.privados %>% 
  filter(grupos.calif != "Ns/Nr",grupos.tamanio != "Ns/Nr",
         Categoria == "Asalariados",Pais == "ARG") %>% 
  group_by(grupos.calif,grupos.tamanio,Pais) %>% 
  summarise(casos = n(),
            casos.pond = sum(PONDERA_SALARIOS),
            salario.mensual.prom = weighted.mean(IOP,PONDERA_SALARIOS),
            salario.mensual.mediana = median(IOP)) %>% 
  arrange(Pais,salario.mensual.prom) %>% 
  group_by(Pais,grupos.calif) %>% 
  mutate(Penalidad.tamanio = salario.mensual.prom/salario.mensual.prom[grupos.tamanio=="Grande"])%>% 
  group_by(Pais,grupos.tamanio) %>% 
  mutate(Penalidad.calif = salario.mensual.prom/salario.mensual.prom[grupos.calif=="Alta"])

ggplot(salario.promedio.calif, aes(x = grupos.calif, 
               y = salario.mensual.prom,
               label = scales::percent(Penalidad.tamanio,accuracy = 1),
               group = grupos.tamanio,
               fill = grupos.tamanio)) + 
  theme_minimal()+
  labs(title = "Salarios en moneda nacional y penalidad por tamaño de empresa, según calificación del puesto",
       subtitle = "Total Asalariados según grupo ",
       y = "Salario en moneda nacional")+
  theme()+
  geom_col(position = "dodge")+
  geom_text(position=position_dodge(width=1),size = 4)+
