...
# openxlsx::write.xlsx(list("Salario Perfiles Arg" = ingresos.eph.asalariados.calif),
#                      file = "Resultados/salario perfiles arg.xlsx")  
...
graf.todos <- EUROPA_USA_ARG %>%
  ...
  filter((ANO4 == 2018 & !(Pais %in% c("DE")))|
           ANO4 == 2017 & Pais %in% c("DE")) %>% 
  ungroup() %>% 
  select(ANO4,COD.ESPANIOL,Orden,Pais,tamanio.calif,tasa.s.desc.jubil,tasa.partime.asal,tasa.temp.asal) %>% 
...
graf.todos %>% 
  filter(Pais != "SE") %>% 
  ggplot(.,
         aes(x = reorder(COD.ESPANIOL,desc(Orden)), y = valor,
