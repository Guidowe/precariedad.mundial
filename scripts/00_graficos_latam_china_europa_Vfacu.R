...
paleta1 <- c(azul[1],
             ...
             verde[1])
###############################Data Encuestas#########################
#perfiles <- readRDS(file = "Resultados/America_RDS")
...
perfiles <- readRDS(file = "Resultados/America_China.RDS")
perfiles <- bind_rows(perfiles, perfiles_europa)


agregado <- readRDS(file = "Resultados/America_China_agregado.RDS")
...
#Cambio nombres de paises en resultados de Europa para que coincidan con tabla CODE
perfiles <- perfiles %>% 
...
America %>% 
  filter(grupos.tamanio== "Pequeño") %>% 
  ggplot(.,
         aes(x = reorder(Pais,Orden), y = tasa.asalarizacion,
             fill = tamanio.calif,group = tamanio.calif,
