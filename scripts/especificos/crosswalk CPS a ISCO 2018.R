...
####Cros a 1 digito manual#####
cross.soc.a.isco.1.digit  <- cross.soc.a.isco %>%
  group_by(SOC,SOC.title,ISCO.1.digit,desc.isco.1,subjetividad) %>%
  summarise(casos = n()) %>% 
  group_by(SOC,SOC.title) %>% 
  mutate(ISCO.1.digit.probs  = casos/sum(casos)) %>% 
  ungroup()

duplicados <- cross.soc.a.isco %>% 
  select(SOC,SOC.title,ISCO.1.digit) %>% 
  unique() %>% 
  group_by(SOC,SOC.title) %>% 
  summarise(casos = n(),
            cuales = paste0(ISCO.1.digit,collapse = " _ ")) %>% 
  filter(casos>1)

cross.soc.a.isco.1.digit.manual <- cross.soc.a.isco.1.digit %>% 
  muate(ISCO.digit = case_when(SOC ="11-1021" ~1,
                               SOC ="11-9021" ~2,
                               SOC ="13-1071" ~2,
                               SOC ="13-1199" ~2))

###Cross a 1 digito sampleado###
cross.census.a.soc.a.isco.nested  <- cross.census.a.soc.a.isco %>% 
...
Base_Usa_cruzada <- Base_USA %>% 
  filter(ASECFLAG==1) %>% 
  select(OCCLY,INCTOT,WKSWORK1,AGE) %>% 
  rename(Census = OCCLY) %>%  
