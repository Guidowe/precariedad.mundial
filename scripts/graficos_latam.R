...
facu <- Resultados %>% 
  rename(grupos.calif = CALIF,
         grupos.tamanio = TAMA,
         Pais = PAIS)

guido <- resultados %>% 
  rename(tasa.noreg.asal = tasa.no.registro)

usa.arg <- PRECARIEDAD %>%
  rename(tasa.noreg.asal = tasa.s.desc.jubil) %>% 
  mutate(Periodo = case_when(Pais == "USA" ~ ANO4-1,
                               TRUE~ANO4)) %>% 
  filter(Periodo == 2018)
...
# ggsave("Resultados/productividad_salarios_GW_filtrado.jpg",
#        width =9,height = 7)
...
graf.todos <- America %>%
  ...
  select(Pais,tamanio.calif,tasa.partime.asal,tasa.temp.asal,tasa.noreg.asal) %>% 
  pivot_longer(cols = 3:ncol(.),
               names_to = "indicador",values_to = "valor") %>% 
  mutate(
    indicador = factor(
      case_when(
        indicador == "tasa.noreg.asal" ~ "No Registro",
...
graf.todos %>% 
  filter(indicador == "Part time involuntario") %>% 
  ggplot(.,
         aes(x = Pais, y = valor,
             fill = indicador,group = indicador,
             label = round(valor,2))) +
...
ingresos.todos %>% 
  ...
  ggplot(.,
         aes(x = tamanio.calif, y = decil.m.promedio,
             fill = tamanio.calif,group = tamanio.calif,
             label = round(decil.m.promedio,1))) +
  geom_col(position = "dodge")+
  geom_text(position = position_dodge(),size=4)+
...
ggsave("Resultados/deciles.jpg",width = 15.59,height = 9)
