...
Variables1719  <- c("CODUSU","NRO_HOGAR","COMPONENTE","ANO4","TRIMESTRE" ,"AGLOMERADO","H15",
    ...
    "PP07A","PP05B2_ANO","PP04B3_ANO","PP07E","NIVEL_ED","PONDIIO","DECOCUR",
...
eph.ocup.privados <- Base_EPH.cat %>% 
  filter(!(PP04B_COD %in% c(83:84,8300:8499)), #Adm Publica
         !(PP04B_COD %in% c(97:98,9700:9899)), #Serv Domestico
         ESTADO == 1) %>% 
  mutate(Pais = "ARG",
         PONDERA_SALARIOS = case_when(ANO4  %in%  2016:2020 ~ PONDIIO,
                                      ANO4  %in%  2003:2015 ~ PONDERA)) %>% 
...
#########Ocup.nivel#############################################
perfiles.ocupados.nivel.ed <- base.unica %>% 
...
indicadores.anuales.ocupados.nivel.ed <- perfiles.ocupados.nivel.ed %>% 
...
#########asal.nivel#############################################
perfiles.asalariados.nivel.ed <- base.unica %>% 
...
indicadores.anuales.asalariados.nivel.ed <- perfiles.asalariados.nivel.ed %>% 
  group_by(grupos.nivel.ed,grupos.tamanio,Pais,ANO4) %>% 
  summarise_all(mean, na.rm = TRUE) %>% 
  select(-TRIMESTRE) %>% 
  arrange(ANO4,Pais,grupos.tamanio)
...
ingresos.eph.asalariados.nivel <- EPH.ingresos.deciles %>% 
  filter(grupos.tamanio != "Ns/Nr",grupos.nivel.ed != "Ns/Nr") %>% 
...
save(insercion.niveles.arg,
  ...
  indicadores.anuales.ocupados.nivel.ed,
  indicadores.anuales.ocupados.calif,
  indicadores.anuales.asalariados.nivel.ed,
  indicadores.anuales.asalariados.calif,
  ingresos.asec.asalariados.nivel,
  ingresos.asec.asalariados.calif,
  ingresos.eph.asalariados.nivel,
  ingresos.eph.asalariados.calif,
  EPH.ingresos.deciles,
