...
table(Base_EPH.cat$part.time.inv)
...
set.seed(68150)    class(eph.ocup.privados$ingreso.mensual)
EPH.ingresos.deciles <- eph.ocup.privados %>%
  ...
  mutate(ingreso.horario = ingreso.mensual/30*7/PP3E_TOT,
         ...
         ingreso.horario.d = ingreso.horario+runif(nrow(.),min = -.01,max =.01)) %>% 
  group_by(Pais,ANO4,TRIMESTRE) %>% 
  mutate(Decil.ing.mensual = statar::xtile(ingreso.mensual.d,n=10,w = PONDERA_SALARIOS),
         Decil.ing.horario = statar::xtile(ingreso.horario.d,
                                           n=10,
                                           w = PONDERA_SALARIOS))
...
ingresos.eph.asalariados.nivel <- EPH.ingresos.deciles %>% 
  ...
  summarise(casos.muestrales=n(),
            ...
            decil.h.promedio = weighted.mean(Decil.ing.horario,
            ...
            decil.m.promedio = weighted.mean(Decil.ing.mensual,
                                             PONDERA_SALARIOS,na.rm = T)) %>% 
  ungroup() %>% 
  mutate(Particip_emp = total/sum(total)*100) %>% 
  ungroup() %>% 
